...
function mapEmpresa(raw: string): string {
  ...
  if (l.includes('nobarzo') || l.includes('novarso') || l.includes('nobarso')) return 'Nobarzo SpA'
...
async function main() {
  ...
  for (const [name, rut, type] of [
    ['Nobarzo SpA', '76000001-1', 'PRINCIPAL'],
  ...
  for (const row of cecoRows) {
    ...
    const companyId = empMap.get(companyName) ?? empMap.get('Nobarzo SpA')!
